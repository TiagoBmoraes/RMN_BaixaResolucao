...
% pulso simples de ângulo theta;
% Tiago Bueno de Moraes  -  (tiagobuemoraes@gmail.com) - Química Nova 2013
%
...
% Espectrômetro
theta = 90;             % (graus) Angulo do Pulso de Radio-Frequência
theta = theta*pi/180;   % convertendo ângulo para radianos;
...
for f=1:length(df)   
  ...
  M(:,1)= Rtheta*M0 + B;             % Aplicação pulso angulo theta
  for k=2:(N0)                       % evolução sinal
	  M(:,k) = E*Rphi*M(:,k-1)+B;
  end;
