...
Tn1    = 50;    % (ms) Tempo até Bloco de 180°
Tn2    = 100;   % (ms) Tempo entre Pulsos de 180° (CPMG: Tn2 = 2*Tn1)
Np     = 8;     % Numero de pulsos de 180°

% Se colocar Np maior do que cabe dentro do tempo de aquisição vai dar
% erro, então, diminuir Np ou aumentar T
...
N1 = round(Tn1/dT);            % n° pontos CP até Bloco  (tau/2)
N2 = round(Tn2/dT);            % n° pontos entre pulsos Bloco

% Caso queria Número de Echo que cabem no Tempo de aquisição:
%Np = floor((N0-(N1+1))/(N2+1));    % nº de pulsos no Bloco
N3 = N0 - (N1+1) - (Np*(N2+1));    % nº pontos depois do ultimo pulso
...
for f=1:length(df)
  ...
  M(:,1)= RthetaCP*M0 + B;             % pulso de thetaºx
  for k=2:N1+1                         % evolução 
	  M(:,k) = E*Rphi*M(:,k-1)+B;
  end;
  N1c=N1+1;  
   
  for n=1:Np
      M(:,N1c+1) = RthetaBloco*M(:,N1c)+B;     % pulso de 180ºx
    for k=1:N2                                 % evolução
